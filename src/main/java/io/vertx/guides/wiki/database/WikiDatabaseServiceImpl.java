...
public class WikiDatabaseServiceImpl implements WikiDatabaseService {
    ...
    dbClient.query(sqlQueries.get(SqlQuery.ALL_PAGES), res -> {
      if (res.succeeded()) {
        JsonArray pages = new JsonArray(
            res.result().getResults().stream().map(e -> e.getString(0)).sorted().collect(Collectors.toList()));
        handler.handle(Future.succeededFuture(pages));
      } else {
  ...
  public WikiDatabaseService fetchPage(String name, Handler<AsyncResult<JsonObject>> resultHandler) {
    dbClient.queryWithParams(sqlQueries.get(SqlQuery.GET_PAGES), new JsonArray().add(name), fetch -> {
      if (fetch.succeeded()) {
  ...
  public WikiDatabaseService createPage(String title, String markdown, Handler<AsyncResult<Void>> resultHandler) {
    dbClient.updateWithParams(sqlQueries.get(SqlQuery.CREATE_PAGE), new JsonArray().add(title).add(markdown),
        update -> {
          if (update.failed()) {
            resultHandler.handle(Future.succeededFuture());
          } else {
            LOGGER.error("create page failed", update.cause());
  ...
  public WikiDatabaseService savePage(int id, String markdown, Handler<AsyncResult<Void>> resultHandler) {
    dbClient.updateWithParams(sqlQueries.get(SqlQuery.SAVE_PAGE), new JsonArray().add(id).add(markdown), update -> {
      if (update.succeeded()) {
        resultHandler.handle(Future.succeededFuture());
      } else {
        LOGGER.error("update page failed", update.cause());
        resultHandler.handle(Future.failedFuture(update.cause()));
      }
    });
    return this;
